...
describe('toBarbellSetup', () => {
  it('hands the plate math the plate unit, the bar, the collars, and the rack', () => {
...
describe('the plate inventory', () => {
  ...
  it('builds a fresh rack rather than handing out the default one', () => {
    // A consumer that persists its own equipment starts from this, and two
    // consumers sharing one array is one of them editing the other's rack.
    expect(defaultInventory('lb')).not.toBe(DEFAULT_EQUIPMENT.inventory.lb);
    expect(defaultInventory('lb')).toEqual(DEFAULT_EQUIPMENT.inventory.lb);
  });

  it('answers the default diameter for a denomination without an inventory', () => {
    // The logbook needs this when it composes a rack a lifter has edited: the
    // question "is a 10 lb plate full diameter here" has to be answerable before
    // there is an `Equipment` to ask it of.
    expect(fullDiameterByDefault('lb', 10)).toBe(true);
    expect(fullDiameterByDefault('kg', 10)).toBe(false);
  });
});
